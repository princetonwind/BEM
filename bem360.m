function [ rho, mu, summ, bldout ] = bem360(rotor, foil, T, P, speed, pitch , U , nb, NE)
%Blade Element Momentum code for HRTF
% [ rho, mu, summ, bldout ] = bem360(rotor, foil, T, P, speed, pitch , U , nb, NE)
...
% rfolder = 'C:\Users\Mark Miller\Documents\Research\Wind Turbine\NACA Foils\NACA Paper Polars\';
...
% Load airfoil data  
        for m = 1:numel(aft)
            ...
            % ( AoA , Re , Foil Type )
            dum = dlmread([rfolder char(aft(m)) '\' char(aft(m)) '_CL.txt'],'\t',1,0);
            Cl(:,:,m) = dum(2:end,2:end);
            Re(:,m) = dum(1,2:end);
            aoa(:,m) = dum(2:end,1).*pi./180; %Convert to radians
            Cd(:,:,m) = dlmread([rfolder char(aft(m)) '\' char(aft(m)) '_CD.txt'],'\t',2,1);
            clear dum
        end
%-------------------%    
%  Enter BEM Code   %
...
%              Rec(jj) = 2.2E6;  %Forces a specific Re
...
bldout = [abr Pt Pn a Clc Cdc alpha phi speed*pi/30.*abr.*(1+ap) U.*(1-a) ap];
end
